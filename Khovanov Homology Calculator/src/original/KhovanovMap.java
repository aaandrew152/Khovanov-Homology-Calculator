   ...
   // private Stack<Map> maps;
    ...
    public void fillResolvedKnots(Knot k){
        if(k.getLeftKnot()==null){
            int numOnes = ArrayListUtils.countMatches(k.getBinCode(), 1);
    ...
    public void setMaps(){       
        ...
        for(int i=0; i<this.rows.length; i++){
            for(int k=0; k<this.rows[i].size(); k++){
                ...
                ArrayList<ArrayList<Integer>> outMaps = current.outMaps();
                for(ArrayList<Integer> s:outMaps){
                    ...
                    if(current.numCircles() < to.numCircles()){ //split map
                        theCircle = new SeifertCircle(new ArrayList<Integer>());
    ...
    public KhovanovMapNode findNode(ArrayList<Integer> code){//returns node based on binary code, throws exception otherwise
        int numOnes = ArrayListUtils.countMatches(code, 1);
